...
| Crash after 4 steps: "Linear solver failed to solve for dU" (e.g. 2019_T2_32m) | Firn core CSV had overlapping depth sections. `read_density_profile()` passed them through; `build_sno_dataframe()` summed all `Layer_Thick` to a value larger than `HS_Last`. SNOWPACK detected the overhang, reorganised the top layers over 4 steps, and produced a degenerate surface layer with `Vol_Frac_I ≈ 0` that made the thermal solver singular. | Fixed in `read_density_profile()`: overlapping layer boundaries are clipped before the SNO is built. `HS_Last` is now set to `sum(Layer_Thick)` (not `bottom_m.max()`) so gaps cannot cause a mismatch. |
| Crash after thousands of steps: "Error solving 'b' with Eigen" in VapourTransport (e.g. 2019_T3_32m) | SNOWPACK writes `Rho_S = 0` on near-surface layers after long-run layer-merge artefacts. `rewrite_sno_profiledate_and_clip_timestamps()` passed `Rho_S` through unchanged. `VapourTransport::LayerToLayer` divides by density → singular matrix. | Fixed: detect `Rho_S ≤ 0` or non-finite in `rewrite_sno_profiledate_and_clip_timestamps()` and recompute as `Vol_Frac_I × ρ_ice + Vol_Frac_W × ρ_water`. |
