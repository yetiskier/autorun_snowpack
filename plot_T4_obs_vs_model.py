...
PRO_PATH   = SCRIPT_DIR / "2022_T4_25m/output/2022-T4-25m_TEMP_ASSIM_RUN.pro"
...
def load_modelled():
    ...
    for ti in range(len(times)):
        ...
        t_s    = t[:n][order]
        surface = h_s[-1]
        # mask dense basal ice (≥900 kg/m³ artifact) — not available here,
        # so just use all layers
        dm = (surface - h_s) / 100.0        # depth from surface in m
        # dm descending; reverse for interp1d (needs ascending x)
        dm_rev = dm[::-1]
        t_rev  = t_s[::-1]
...
fig.suptitle("T4 (1873 m)  —  observed vs modelled firn temperature",
             fontsize=FS, y=1.01)
...
out = SCRIPT_DIR / "T4_obs_vs_model.png"
