...
void vanGenuchten::SetVGParamsSnow(const VanGenuchten_ModelTypesSnow VGModelTypeSnow, const K_Parameterizations K_PARAM, const bool& matrix, const bool& seaice)
{
	if (EMS->theta[ICE] > 0.75) {
		...
		if(matrix==true) {
			// Use density-dependent residual water content (Coleou & Lesaffre 1998) as the
			// physical upper bound, replacing the hard-coded 0.02 cap so that RE retains
			// the same amount of water as BUCKET at a given firn density.
			const double theta_r_phys = ElementData::snowResidualWaterContent(EMS->theta[ICE]);
			const double TuningFactor=0.75;				//Tuning factor for scaling
			//Increase theta_r in case of wetting:
			theta_r=std::max(0., std::min(theta_r_phys, std::max(theta_r, TuningFactor*EMS->theta[WATER])));
