...
inline void Usage(const string& programname)
{
	...
	cout << "Usage: " << programname << endl
		...
		<< "\t[-d, --daemon] Persistent daemon mode: read RELOAD_SNO/RUN/QUIT commands from stdin\n"
...
inline void parseCmdLine(int argc, char **argv, string& begin_date_str, string& end_date_str)
{
	...
	while ((opt=getopt_long( argc, argv, ":b:e:m:c:s:d:v:h", long_options, &longindex)) != -1) {
		switch (opt) {
		case 0:
			break;
		case 'b': {
...
inline void real_main (int argc, char *argv[])
{
	...
	for (size_t i_stn=0; i_stn<vecStationIDs.size(); i_stn++) {
		...
		// DAEMON OUTER LOOP — executes exactly once in normal mode, repeatedly in daemon mode
		bool _daemon_active = true;
		bool _first_chunk = true;
		while (_daemon_active) {
			// Per-chunk hazard initialisation (duration shrinks with each daemon chunk)
			qr_Hdata.clear();
			qr_Hdata_ind.clear();
			const double duration = (dateEnd.getJulian() - current_date.getJulian() + 0.5/24)*24*3600;
		...
		if (!daemon_mode) {
			// Normal mode: one pass then done
			_daemon_active = false;
		} else {
			// Daemon mode: signal Python this chunk is done, then wait for the next command
			std::cout << "CHECKPOINT " << current_date.toString(mio::Date::ISO) << "\n";
			std::cout.flush();

			bool _got_next = false;
			std::string _cmd;
			while (std::getline(std::cin, _cmd)) {
				// Strip trailing CR / LF / space
				while (!_cmd.empty() && (_cmd.back()=='\r' || _cmd.back()=='\n' || _cmd.back()==' '))
					_cmd.pop_back();
				...
				if (_cmd == "QUIT") {
					...
					_got_next = true;
					break;
				} else if (_cmd == "RELOAD_SNO") {
					// Re-read the (Python-modified) SNO; IOManager buffer stays warm
					mio::Date _reload_date; // undefined → readSlopeMeta fills from SNO ProfileDate
					if (!readSlopeMeta(io, snowpackio, cfg, i_stn, slope, _reload_date,
					                   vecSSdata, vecXdata, sn_Zdata, Mdata,
					                   wind_scaling_factor, time_count_deltaHS)) {
						std::cerr << "[E] DAEMON: RELOAD_SNO failed — terminating\n";
						_daemon_active = false;
						_got_next = true;
						break;
					}
					current_date = _reload_date;
					memset(&mn_ctrl, 0, sizeof(MainControl));
					// Now wait for RUN command to update dateEnd
				} else if (_cmd.size() > 9 && _cmd.substr(0, 9) == "SETTEMPS ") {
					// Direct layer temperature injection: "SETTEMPS i0:T0,i1:T1,..."
					// Indices are 0=bottom (matching Edata order); temperatures in Kelvin.
					SnowStation& Xdata = vecXdata[slope.mainStation];
					std::istringstream ss(_cmd.substr(9));
					std::string tok;
					while (std::getline(ss, tok, ',')) {
						const size_t colon = tok.find(':');
						if (colon == std::string::npos) continue;
						const size_t idx = static_cast<size_t>(std::stoul(tok.substr(0, colon)));
						const double T_K  = std::stod(tok.substr(colon + 1));
						if (idx < Xdata.getNumberOfElements()) Xdata.Edata[idx].Te = T_K;
					}
					std::cout << "READY\n";
					std::cout.flush();
					// Now wait for RUN command
				} else if (_cmd.size() > 4 && _cmd.substr(0, 4) == "RUN ") {
					mio::IOUtils::convertString(dateEnd, _cmd.substr(4), i_time_zone);
					_got_next = true;
					break;
				}
			}
			if (!_got_next) _daemon_active = false; // stdin EOF
		}

		} // end daemon outer loop
