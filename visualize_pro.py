"""
Visualize SNOWPACK PRO output for any site.
...
# ---------------------------------------------------------------------------
# Paths — set dynamically from CLI args in main()
# ---------------------------------------------------------------------------
SCRIPT_DIR = Path(__file__).resolve().parent   # autorun_snowpack/
...
def build_paths(year: int, site: str, depth: int) -> dict:
    ...
    project_dir = SCRIPT_DIR / site_id
    output_dir  = project_dir / "output"
    return {
        "pro_file":       output_dir / pro_name,
        "obs_file":       SCRIPT_DIR / "AllCoreDataCommonFormat"
                          / "Concatenated_Temperature_files"
                          / f"{site_id}_Tempconcatenated.csv",
        "out_fig":        output_dir / f"{site_id}_visualization.png",
        "out_fig_micro":  output_dir / f"{site_id}_microstructure.png",
        "out_fig_grains": output_dir / f"{site_id}_grain_type.png",
...
def main():
    p = argparse.ArgumentParser(
        description="Visualize SNOWPACK PRO output for a firn-core site.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--site",  default="T3",  help="Site name, e.g. T3")
    p.add_argument("--year",  type=int, default=2022, help="Drill year, e.g. 2022")
    p.add_argument("--depth", type=int, default=25, help="String depth (m), e.g. 25")
    args = p.parse_args()

    paths = build_paths(args.year, args.site, args.depth)
    PRO_FILE       = paths["pro_file"]
    OBS_FILE       = paths["obs_file"]
    OUT_FIG        = paths["out_fig"]
    OUT_FIG_MICRO  = paths["out_fig_micro"]
    OUT_FIG_GRAINS = paths["out_fig_grains"]
    site_label     = paths["site_label"]
    ...
    depth_grid = np.arange(0, args.depth + 0.05, 0.05)
    ...
    fig.suptitle(f"SNOWPACK – {site_label}  (RICHARDSEQUATION scheme)",
    ...
    fig2.suptitle(f"SNOWPACK – {site_label}  microstructure  (RICHARDSEQUATION scheme)",
                  fontsize=13, fontweight="bold", y=0.995)
    ...
    fig3.suptitle(f"SNOWPACK – {site_label}  grain type  (RICHARDSEQUATION scheme)",
